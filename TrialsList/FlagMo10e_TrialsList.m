function trialsList = FlagMo10e_TrialsList( S, P )
...
%  Returns trialsList - list of parameters to substitute over trials
        ...
        for zzk = 1:Orinum 
            ...
            %*******
            for ak = 1:(5*Orinum)  % fifths, 1/5 same, 1/5 move+same, 1/5 move+change, 1/5 blank, 1/5 fixation stim
                   ...
                   if (ak > (1*Orinum))
                       if (ak <= (3*Orinum))       
                         tat = ((ak-(Orinum))-1); % not used exactly, +90 or -90 later
                         if ( mod(tat,Orinum) < (floor(Orinum/2)) )
                             ora = oro - 90;
                             if (ora < 0)
                                 ora = ora + 360;
                             end
                   ...
                   if ( (ak > Orinum) & (ak <= (2*Orinum) ) )  % move only trials
                       fixslot = -1;  
                   end
