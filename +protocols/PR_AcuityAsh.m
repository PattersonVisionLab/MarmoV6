classdef PR_AcuityAsh < handle
  ...
  methods (Access = public)
    function o = PR_AcuityAsh(winPtr)
      o.winPtr = winPtr; 
      o.trialsList = [];
    end
    ...
    function initFunc(o,S,P)
         ...
         for k = 1:P.apertures
            o.hChoice{k} = stimuli.circles(o.winPtr); % choice grating, right (vertical)
            ango = (((k-1)/P.apertures)*2*pi) + (pi/4);
            xpos = P.ecc * cos(ango);
            ypos = P.ecc * sin(ango);
            o.hChoice{k}.position = [(S.centerPix(1) + round(xpos * S.pixPerDeg)), ...
                                     (S.centerPix(2) + round(ypos * S.pixPerDeg))]; 
            o.hChoice{k}.size = round(P.choiceRad * S.pixPerDeg);
            o.hChoice{k}.colour = P.bkgd - floor( P.choiceCon * 1.27);
            o.hChoice{k}.weight = P.choiceWidth; %width in pixels
         end
    ...
    function [FP,TS] = prep_run_trial(o)
          ...
          if o.faceTrial
              o.fixDur = o.fixDur + o.P.faceHoldExtra;
          end
          o.faceFlash = false;
          ...
          o.flashCounter = 0;
          o.flashCounterMax = floor(o.P.flashFrameLength/2) + floor( rand * o.P.flashFrameLength);
          ...
          if (o.faceTrial)
              o.Iti = o.P.faceIti;  % wait longer to re-establish fixation
          end
    ...
    function drop = state_and_screen_update(o,currentTime,x,y) 
        ...
        if o.state == 3 && currentTime > o.stimStart + o.P.stimHold
            if o.faceTrial
                o.state = 7;  % move to reward and show the face
                o.itiStart = GetSecs;            
            else
        ...
        switch o.state
            case 0
                ...
                o.flashCounter = mod(o.flashCounter+1,o.flashCounterMax);
                if o.flashCounter == 0
                    o.showFix = ~o.showFix;
                    if o.faceTrial
                      if (rand < o.P.faceProbFlash)
                        o.faceFlash = true;
                      else
                        o.faceFlash = false;
                      end
            ...
            case 4    % disappear fixation and show apertures to go
                ...
                if ~o.faceTrial
                    if ( currentTime < o.stimStart + o.P.stimDur )
                      ...
                      for k = 1:o.P.apertures
                        o.hChoice{k}.beforeFrame();
                      end
                    end
                end
            ...
            case {6 7} % once saccade landed, reappear stimulus,  show correct option
                ...
                if o.faceTrial
                   ...
                   if (o.error == 0)  % no error, show stim again
                      o.hProbe.beforeFrame(); 
                      % Aperture choice stimuli shown
                      for k = 1:o.P.apertures
                         o.hChoice{k}.beforeFrame();
