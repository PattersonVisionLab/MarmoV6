classdef PR_Forage < handle
  ...
  properties (Access = public) 
       ...
       rewardGap   = 0;       % gap for next target onset
       rewardTime   = 0;      % store time of last reward
  end
  ...
  properties (Access = private)
    ...
    noiseCap = 0;      % cap for number of items to show
    ...
    SpeedHistory = []; % list of noise frames, adds a field for dot speed
  ...
  methods (Access = public)
    ...
    function initFunc(o,S,P)
       ...
       %******* and a different on duration of stimulus 
       %**********
       if (o.noisetype == 4) % use full field motion background
           %*************
           o.NoiseHistory = zeros(o.MaxFrame,2);
       ...
       if (o.noisetype == 5) % use Spatial background
           ...
           o.SpeedHistory = zeros(o.MaxFrame,(1+o.noiseNum));  % store extra field, speed of dots
           o.hNoise = zeros(7,o.noiseNum);
           ...
           for kk = 1:o.noiseNum
               ...
               %*******
               spd = randi(P.snoisespds);
               noisespeed = P.snoisebase^(1+spd);
               o.hNoise(7,kk) = spd;
               %*******
               o.hNoise(3,kk) = (noisespeed / S.frameRate) * cos(ang);
               o.hNoise(4,kk) = (noisespeed / S.frameRate) * sin(ang);
       ...
       %**********************************************************
       if (o.noisetype == 6) % use full field motion background
           %*************
           o.NoiseHistory = zeros(o.MaxFrame,2);
           o.noiseNum = P.noisenum * P.numSpeed;
           o.hNoise = cell(1,o.noiseNum);
    ...
    function P = next_trial(o,S,P)
          ...
          %****** vary the dot density trial by trial
          if (o.noisetype == 5)
             rd = randi(4)-1;
             % o.noiseCap = floor( o.noiseNum / (2^rd));
             o.noiseCap = floor( o.noiseNum / (sqrt(2)^rd));  %9/12/20 
             for kk = 1:o.noiseCap
                 life = mod(kk, P.snoiselife);
                 o.hNoise(6,kk) = life;
             end
             for kk = (o.noiseCap+1):o.noiseNum
                o.hNoise(6,kk) = NaN;
             end
          end
    ...
    function updateNoise(o,xx,yy,currentTime)
         if (o.FrameCount < o.MaxFrame)  
            ...
            if (o.noisetype == 5)  
                ...
                for kk = 1:o.noiseNum
                   if (kk <= o.noiseCap)
                     ...
                     if (o.hNoise(6,kk) > o.P.snoiselife)
                       ...
                       noisespeed = o.P.snoisebase^(1+spd);
                       o.hNoise(7,kk) = spd;
                       %****
                       o.hNoise(3,kk) = (noisespeed / o.S.frameRate) * cos(ang);
                       o.hNoise(4,kk) = (noisespeed / o.S.frameRate) * sin(ang);
                     else
                   ...
                   snlist(1,kk) = o.hNoise(7,kk);
                   %********************
                end
                %****** plot dots
                for kk = 1:o.noiseNum
                   if ~isnan(o.hNoise(6,kk))
                     sx = o.hNoise(1,kk);
                     sy = o.hNoise(2,kk);
                     lifer = o.hNoise(6,kk);
                     midlife = floor(o.P.snoiselife/2);
                     if (lifer < (midlife+1))
                        col = 127 + floor(o.P.range * (lifer/midlife)^1);
                     else
                        col = 127 + floor(o.P.range * ((o.P.snoiselife-lifer)/midlife)^1);    
                     end
                     position = [(o.S.centerPix(1) + round(sx*o.S.pixPerDeg)),...
                                 (o.S.centerPix(2) - round(sy*o.S.pixPerDeg))];
                     r = round((o.P.snoisediam/2)*o.S.pixPerDeg);
                ...
                o.FrameCount = o.FrameCount + 1;
                o.NoiseHistory(o.FrameCount,:) = [NaN nlist];  % first element time, others x,y positions
                o.SpeedHistory(o.FrameCount,:) = [NaN snlist];  % first element time, others x,y positions
                %********** 
            end
            %*************** 
            if (o.noisetype == 6) 
                ...
                if (cycle == 1)  % set new target
                    kk = randi(o.noiseNum);  %positive value onset
                    %****** this is where you would have to copy over
                    %****** dot locations to avoid dot onset response
                    if (o.noiseStim > 0)
                       o.hNoise{kk}.x = o.hNoise{o.noiseStim}.x;
                       o.hNoise{kk}.y = o.hNoise{o.noiseStim}.y;
                    end
                    %***********************
                    o.noiseStim = kk;
                end
                if (cycle <= o.P.stimCycle)
                   ...
                   %**** here will still show it, but not update the motion 
                   if (o.noiseStim > 0)
                       o.hNoise{o.noiseStim}.beforeFrame(); 
                   end
                   %************
    ...
    function keepgoing = continue_run_trial(o,screenTime)
        ...
        %********* static dots till motion (but dots will reset)
        %********* how can we stop the reset ... on first frame
        %***** where noiseStim changes, copy over dot locations?
        if (o.noisetype == 6)
            cycle = mod( o.FrameCount, o.P.totCycle );
            if (cycle > 0) && (cycle < o.P.stimCycle)
                if (o.noiseStim > 0) 
                   o.hNoise{o.noiseStim}.afterFrame;
                end
            end
    ...
    function PR = end_plots(o,P,A)   %update D struct if passing back info
        ...
        if o.FrameCount == 0
            ...
            if ~isempty(o.SpeedHistory)
               PR.SpeedHistory = o.SpeedHistory(1:o.FrameCount,:);
            end
