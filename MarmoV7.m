function varargout = MarmoV7(varargin)
% MarmoV7 M-file for MarmoV7.fig
% Updated 2026-06-15 from MarmoV6
%
%      THIS IS MarmoV7 VERSION 1C, THIS CORRESPONDS TO THE VERSION TEXT
%      IN THE MarmoV7.fig FILE
%
...
function MarmoV6_OpeningFcn(hObject, eventdata, handles, varargin)
    ...
    handles.A = handles.C;
    % Write to eye tracker panel and grab gains
    ...
    % Add plot handles to A in case handles isn't available (e.g. protocol runs)
    ...
    % OPEN UP COMMUNICATION WITH THE PUMP FOR REWARD DELIVERY
    if handles.S.newera %
    ...
    elseif handles.S.solenoid
        ...
        vol = sprintf('%d ms', S.pumpDefVol * 1e3);
        set(handles.JuiceVolumeText, 'String', vol); % mL!!
    else
...
function Initialize_Callback(hObject, eventdata, handles)
    ...
    set([handles.ChooseSettings, handles.Initialize, handles.OutputSubjectEdit],...
        "Enable", "off");
    guidata(hObject, handles); drawnow;
    
    % Load the protocol settings
    %cd(handles.settingsPath);
    protocolSettingsFcn = str2func(handles.settingsFile(1:end-2));
    [handles.S, handles.P] = protocolSettingsFcn();
    ...
    % Initialize the protocol (PR)
    protocolFcn = str2func(handles.S.protocolClass);
    handles.PR = protocolFcn(handles.A.window);
    ...
    % Generate and initialize background image protocol (PRI)
    cd(handles.settingsPath);
    ...
    % Generate the file name and iterate suffix to find a new file
    ...
    while isfile(fullfile(handles.outputPath, handles.A.outputFile))
    ...
    % For highlighted parameter, provide a description and editable value
    ...
    % Blank the eyetrace plot (TODO: line handles)
    h = handles.EyeTrace;
...
function ClearSettings_Callback(hObject, eventdata, handles)
    ...
    [c, dx, dy, rot] = grabCalibrationFromStruct(handles.A);
    if ~handles.S.DummyEye
    ...
    updateVolumeDisplay(handles.JuiceVolumeText, S, handles.A.juiceVolume * 1e3);
...
function RunTrial_Callback(hObject, eventdata, handles)
    ...
    set([handles.RunTrial, handles.FlipFrame, handles.Background_Image,...
        ...
        handles.OutputPrefixEdit, handles.OutputDateEdit,...
    ...
    % MOVE TASK RELATED STRUCTURES OUT OF HANDLES FOR THE RUN LOOP -- this
    % way if a callback interrupts the run task function, we can update any
    % changes the interrupting callback makes to handles without affecting
    % those task related structures. E.g. we can run the task using
    % parameters as they were at the start of the trial, while getting
    % ready to cue any changes the user made on the next trial.
    ...
    if ~exist(fullfile(handles.outputPath, A.outputFile), 'file')
        cd(handles.outputPath);             
        save(A.outputFile,'S');     % save settings struct to output file
        cd(handles.taskPath); 
    end              
        
    % This is here in case user updated things during "Pause"
    ...
    while handles.runTask && A.j <= A.finish
        % See notes on callback execution and handle management in design notes
        
        % Should we run a background image trial
        ...
        % Update in case juice volume was set in parameters
        if handles.A.juiceVolume ~= A.juiceVolume
            handles.reward.volume = A.juiceVolume;
            updateVolumeDisplay(handles.JuiceVolumeText, handles.S, A.juiceVolume * 1e3);
            handles.A.juiceVolume = A.juiceVolume;
        end
        
        % UPDATE HANDLES FROM ANY CHANGES DURING NEXT TRIAL -- IF THIS
        % ISN'T DONE, THEN THE OTHER CALLBACKS WILL BE USING A DIFFERENT
        % HANDLES STRUCTURE THAN THIS LOOP IS
        guidata(hObject,handles);
        ...
        % load values for plotting (FP) and TimeSensitive states (TS)
        ...
        syncOnsetTimeStamps(handles.S, handles.A, handles.eyetrack, STARTCLOCK);
        ...
        % Info for timing reward delivery with low graphics demands
        screenTime = GetSecs();
        minTimeForDrop = (0.5 / handles.S.frameRate);
        maxFramesToDrop = 12;
        holdrop = 0;
        dropreject = 0;
    
        while runloop  % FRAME LOOP STARTS
            if ~handles.runImage
            ...
            %% Pass to protocol for next frame
            % Display next frame and determine if reward should be delivered
            if ~handles.runImage
            ...
            %% Reward delivery
            % Only deliver drop if there is time before the next screen flip, 
            % to a limit of "maxFramesToDrop"
            if drop > 0
                holdrop = 1;
                dropreject = 0;
            end
            if holdrop
                droptime = GetSecs();
                if ((droptime-screenTime) < minTimeForDrop) || (dropreject > maxFramesToDrop)
                    holdrop = 0;
            ...
            if updateGUI  
                ...
                A.rot = handles.A.rot;
                handles.FC.update_eye_calib(A.c, A.dx, A.dy, A.rot);
                fprint('Update eye calibration time = %s\n', toc);
            end
            if ~handles.runImage
        ...
        %% Signal trial end
         % set screen to gray, trial over, start ITI
        ENDCLOCK = handles.FC.last_screen_flip();  
        ENDCLOCKTIME = GetSecs();

        syncOffsetTimeStamps(S, handles.A, handles.eyetrack, ENDCLOCK);
        ...
        % During ITI is a good time for updating plots amd UI
        itiStart = GetSecs();
        ...
        handles.FC.plot_eye_trace_and_flips(handles);  
        ...
        % -----------------------------------------------------------------
        %% Create trial data struct

        D = struct();
        D.P = P;  % Trial parameters
        ...
        %% Save the trial data struct
        cd(handles.outputPath);
        Dstring = sprintf('D%d', A.j);  % store data in variable with this name
        ...
        if handles.A.juiceVolume ~= A.juiceVolume
            txt = handles.reward.getVolumeText(A.juiceVolume);
            set(handles.JuiceVolumeText, 'String', txt);
        end
    
        % Update task-related structures in case leaving loop
        handles.A = A;
        if ~handles.runImage
            handles.S = S; handles.P = P;
        else
            handles.SI = S; handles.PI = P;
        end
    
        % If it was an interleave Image trial, set it back proper
        if SetRunBack == 1
            handles.runImage = false;
            SetRunBack = 0; CorCount = 0;
            S = handles.S; P = handles.P;
        end
    
        % Update the parameter list to show the next trial's parameters
        updateParameterDisplay(handles);
    
        % UPDATE THE HANDLES STRUCTURE FROM ALL OF THESE CHANGES
        guidata(hObject,handles);
        % ALLOW OTHER CALLBACKS INTO THE THE QUEUE. IF PARAMETERS ARE
        % CHANGED BY CHANCE THIS LATE IN THE LOOP, THEY WILL NOT BE CHANGED
        % UNTIL REACHING THE END OF THE NEXT TRIAL, BECAUSE P HAS ALREADY
        % BEEN ESTABLISHED FOR THE NEXT TRIAL. IF YOU EXIT THE LOOP,
        % THOUGH, THEN P WILL BE UPDATED BY WILL BE UPDATED BY ANY CHANGES
        % TO THE HANDLES
        pause(.001); handles = guidata(hObject);
    
        % Ensure protocol run stops if at end or if paused.
    ...
    % UPDATE THE PARAMETERS LIST IN CASE OF ANY CHANGES MADE AFTER RUNNING
    % THE END TRIAL COMMAND
    updateParameterDisplay(handles);
    ...
    set([handles.RunTrial, handles.FlipFrame, handles.Background_Image,...
        handles.Calib_Screen, handles.CloseGui, handles.ClearSettings,...
        handles.OutputPrefixEdit, handles.OutputDateEdit, handles.OutputSuffixEdit],...
        'Enable', 'On');
    set([handles.Parameters, handles.TrialMaxEdit, handles.JuiceVolumeEdit,...
        handles.ChooseSettings, handles.Initialize, handles.ParameterEdit],...
        'Enable', 'On');
...
function Background_Image_Callback(hObject, eventdata, handles)
    % Idea is the following, turn on flag and run PRI object instead of the
    % PR object, otherwise data logging and other tracking identical
    handles.runImage = true;
    handles.runOneTrial = true; % keep running till paused, or true stop at one
    guidata(hObject,handles);
    RunTrial_Callback(hObject, eventdata, handles)
...
function CloseGui_Callback(hObject, eventdata, handles)
    ...
    % Close the reward delivery device
    if S.newera
...
function handles = UpdateOutputFilename(handles)
    ...
    if (~isempty(handles.outputPrefix) && ~isempty(handles.outputSubject) && ...
        ...
        handles.A.outputFile = strcat(handles.outputPrefix, '_',...
            ...
            handles.outputSuffix, '.mat');
        set(handles.OutputFile,'String',handles.A.outputFile);
        % If the file name already exists, provide a warning that data will
        % be overwritten
        ...
        % Note that a new output file is being used. For example, someone
        % might want to be sure the trials list is started over if the
        % output file name changes. Currently I don't have any protocols
        % implementing this.
        handles.A.newOutput = 1;
    else
...
function [c, dx, dy, rot] = grabCalibrationFromStruct(S)
    c = S.c; dx = S.dx; dy = S.dy; rot = S.rot;

%% Critical support functions 
function syncOnsetTimeStamps(S, A, eyetrack, clockTime)
    % Send all bits on to mark trial start
    if S.Datapixx
        datapixx.strobe(63, 0);  % 2 ms
    end

    if S.arrington
        txt = sprintf('TRIALSTART:TRIALNO:%5i %2d %2d %2d %2d %2d %2d',...
            clockText, A.j, clockTime(1:6));   
        eyetrack.sendcommand(txt);
    end

    % This takes about 2 ms, but not used for time strobe
    if S.Datapixx 
        for k = 1:6
            datapixx.strobe(clockTime(k), 0);
        end
    end

function syncOffsetTimeStamps(S, A, eyetrack, clockTime)

    % Send all bits on except for one
    if S.Datapixx
        datapixx.strobe(62, 0);  % 2 ms
    end
    
    % This is a place for timing event to synch up trial ends        
    if S.eyetrack_arrington
        tstring = sprintf('TRIALENDED:TRIALNO:%5i %2d %2d %2d %2d %2d %2d',...
            A.j, clockTime(1:6));   % code the sixlet
        eyetrack.sendcommand(tstring);  % 2 ms
    end
...
function CondenseAppendedData(hObject, handles)
    ...
    % crashes, you would still be able to call this on what is saved
    ...
    if exist(A.outputFile,'file')
        ...
        D = cell(1,1);
        ND = length(fields(zdata));    % include all trials, minus 1 for S
        for k = 1:(ND-1)
