classdef agrating < handle
  ...
  methods (Access = public)
    function o = agrating(winPtr,varargin) % marmoview's initCmd?
      o.winPtr = winPtr;
      o.tex = [];
      o.texRect = [];
      o.goRect = [];
    ...
    function updateTextures(o)  
       ...
       if isinf(o.radius)
          ...
          if (o.aspect > 1)
            ...
            tX = sin(o.orientation*pi/180) * X + ...
                 cos(o.orientation*pi/180) * Y;
            tY = cos(o.orientation*pi/180) * X - ...
                 sin(o.orientation*pi/180) * Y;
            e1 = exp(-.5*( (tX.^2)*(o.aspect^2) + tY.^2)/sigma^2);
          else
